...
namespace pointcloud_to_laserscan
{
        ...
        : Node("pointcloud_to_laserscan", options)
    {
        ...
        tolerance_ = this->declare_parameter("transform_tolerance", 0.0);
        ...
        subscription_listener_thread_ = std::thread(std::bind(&PointCloudToLaserScanNode::subscriptionListenerThreadLoop, this));
    }
    ...
    void pointcloud_to_laserscan::PointCloudToLaserScanNode::cloudCallback(sensor_msgs::msg::PointCloud2::ConstSharedPtr cloud_msg)
    {
        ...
        if (scan_msg->header.frame_id != cloud_msg->header.frame_id)
        {
            ...
            {
                RCLCPP_ERROR(this->get_logger(), "Error when transforming pointcloud: %s", ex.what());
                return;
            }
        }
             ...
             iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z)
        {
            ...
            {
                RCLCPP_DEBUG(this->get_logger(), "rejected for height (%f) out of range [%f, %f]\n", *iter_z, min_height_, max_height_);
                continue;
            }
            ...
            {
                RCLCPP_DEBUG(this->get_logger(), "rejected for angle (%f) outside scan range [%f, %f]\n", angle, scan_msg->angle_min, scan_msg->angle_max);
                continue;
            }
    ...
    void pointcloud_to_laserscan::PointCloudToLaserScanNode::subscriptionListenerThreadLoop()
    {
        ...
        while (rclcpp::ok(context) && alive_.load())
        {
            ...
            if (subscription_count > 0)
            {
                ...
                {
                    RCLCPP_INFO(this->get_logger(), "got a subscriber to laserscan, Subscribing to pointcloud topic");
            ...
            {
                RCLCPP_INFO(this->get_logger(), "no subscribers to laserscan ,shut dowm pointcloud topic");
                sub_.unsubscribe();
            }
